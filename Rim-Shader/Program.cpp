...
void Program::genFragmentBuffers()
{
	...
	glGenBuffers(1, &_uboRim);
	glBindBuffer(GL_UNIFORM_BUFFER, _uboRim);
	glBufferData(GL_UNIFORM_BUFFER, 24, nullptr, GL_DYNAMIC_DRAW);
	GLuint rimBlockIndex = glGetUniformBlockIndex(_programID, "Rim");
	glUniformBlockBinding(_programID, rimBlockIndex, 4);
	glBindBufferBase(GL_UNIFORM_BUFFER, 4, _uboRim);
...
void Program::bindFragmentBuffers(bool useTexture, glm::vec3 viewPosition, const Material& material, const Light& light, const Rim& rim)
{
	glBindBuffer(GL_UNIFORM_BUFFER, _uboFragment);
	...
	glBindBuffer(GL_UNIFORM_BUFFER, _uboRim);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, 12, glm::value_ptr(rim.rimColor));
	glBufferSubData(GL_UNIFORM_BUFFER, 12, 4, &rim.rimPower);
	int useSmoothstepInt = rim.useSmoothstep ? 1 : 0;
	glBufferSubData(GL_UNIFORM_BUFFER, 16, 4, &useSmoothstepInt);
	glBufferSubData(GL_UNIFORM_BUFFER, 20, 4, &rim.rimStrength);
